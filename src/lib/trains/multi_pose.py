    ...
    def forward(self, outputs, batch):
        ...
        for s in range(opt.num_stacks):
            ...
            if opt.eval_oracle_kps:
                if opt.dense_hp:
                    ...
                    output['hps'] = torch.from_numpy(
                        gen_oracle_map(batch['hps'].detach().cpu().numpy(), batch['ind'].detach().cpu().numpy(),
                                       opt.output_res, opt.output_res)).to(opt.device)
            if opt.eval_oracle_hp_offset:
                output['hp_offset'] = torch.from_numpy(
                    gen_oracle_map(batch['hp_offset'].detach().cpu().numpy(), batch['hp_ind'].detach().cpu().numpy(),
                                   opt.output_res, opt.output_res)).to(opt.device)
            ...
            if opt.dense_hp:
                ...
                hp_loss += self.crit_kp(output['hps'], batch['hps_mask'], batch['ind'], batch['hps']) / opt.num_stacks
            if opt.wh_weight > 0:
...
class MultiPoseTrainer(BaseTrainer):
    ...
    def save_result(self, output, batch, results):
        ...
        dets_out = multi_pose_post_process(dets.copy(), batch['meta']['c'].cpu().numpy(),
