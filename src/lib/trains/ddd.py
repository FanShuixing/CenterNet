        ...
        for s in range(opt.num_stacks):
            ...
            if opt.eval_oracle_dep:
                output['dep'] = torch.from_numpy(
                    gen_oracle_map(batch['dep'].detach().cpu().numpy(), batch['ind'].detach().cpu().numpy(),
            ...
            if opt.rot_weight > 0:
                rot_loss += self.crit_rot(output['rot'], batch['rot_mask'], batch['ind'], batch['rotbin'],
...
class DddTrainer(BaseTrainer):
    ...
    def _get_losses(self, opt):
        loss_states = ['loss', 'hm_loss', 'dep_loss', 'dim_loss', 'rot_loss', 'wh_loss', 'off_loss']
        loss = DddLoss(opt)
    ...
    def debug(self, batch, output, iter_id):
        ...
        dets = ddd_decode(output['hm'], output['rot'], output['dep'], output['dim'], wh=wh, reg=reg, K=opt.K)
        ...
        dets_gt = ddd_post_process(batch['meta']['gt_det'].detach().numpy().copy(), batch['meta']['c'].detach().numpy(),
                                   batch['meta']['s'].detach().numpy(), calib, opt)
        ...
        for i in range(1):
            ...
            debugger.add_bird_views(dets_pred[i], dets_gt[i], center_thresh=opt.center_thresh, img_id='bird_pred_gt')
