...
class DddDetector(BaseDetector):
    def __init__(self, opt):
        ...
        self.calib = np.array(
            [[707.0493, 0, 604.0814, 45.75831], [0, 707.0493, 180.5066, -0.3454157], [0, 0, 1., 0.004981016]],
            dtype=np.float32)
    ...
    def process(self, images, return_time=False):
        with torch.no_grad():
            ...
            dets = ddd_decode(output['hm'], output['rot'], output['dep'], output['dim'], wh=wh, reg=reg, K=self.opt.K)
        if return_time:
    ...
    def post_process(self, dets, meta, scale=1):
        ...
        detections = ddd_post_process(dets.copy(), [meta['c']], [meta['s']], [meta['calib']], self.opt)
    ...
    def show_results(self, debugger, image, results):
        debugger.add_3d_detection(image, results, self.this_calib, center_thresh=self.opt.vis_thresh, img_id='add_pred')
        debugger.add_bird_view(results, center_thresh=self.opt.vis_thresh, img_id='bird_pred')
