...
class ExdetDetector(BaseDetector):
    ...
    def debug(self, debugger, images, dets, output, scale=1):
        ...
        for i in range(1):
            ...
            for p in parts:
                tag = 'hm_{}'.format(p)
                pred = debugger.gen_colormap(output[tag][i].detach().cpu().numpy(), (inp_height, inp_width))
                if p != 'c':
    ...
    def post_process(self, dets, meta, scale=1):
        ...
        dets[0, :, 0:2] = transform_preds(dets[0, :, 0:2], meta['c'], meta['s'], (out_width, out_height))
        dets[0, :, 2:4] = transform_preds(dets[0, :, 2:4], meta['c'], meta['s'], (out_width, out_height))
        dets[:, :, 0:4] /= scale
        return dets[0]

    def merge_outputs(self, detections):
        detections = np.concatenate([detection for detection in detections], axis=0).astype(np.float32)
