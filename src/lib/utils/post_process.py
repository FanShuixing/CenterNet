    ...
    for i in range(dets.shape[0]):
        ...
        for j in range(opt.num_classes):
            ...
            top_preds[j + 1] = np.concatenate([
                ...
                get_pred_depth(dets[i, inds, 11:12]).astype(np.float32), dets[i, inds, 12:15].astype(np.float32)
            ],
                                              axis=1)
            if include_wh:
                top_preds[j + 1] = np.concatenate([
...
def ctdet_post_process(dets, c, s, h, w, num_classes):
    ...
    for i in range(dets.shape[0]):
        ...
        for j in range(num_classes):
            inds = (classes == j)
            top_preds[j + 1] = np.concatenate(
                [dets[i, inds, :4].astype(np.float32), dets[i, inds, 4:5].astype(np.float32)], axis=1).tolist()
        ret.append(top_preds)
    return ret
...
def multi_pose_post_process(dets, c, s, h, w):
    ...
    for i in range(dets.shape[0]):
        ...
        top_preds = np.concatenate(
            [bbox.reshape(-1, 4), dets[i, :, 4:5], pts.reshape(-1, 34)], axis=1).astype(np.float32).tolist()
