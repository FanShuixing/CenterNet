    ...
    def __init__(self, node_kernel, out_dim, channels, up_factors):
        ...
        for i, c in enumerate(channels):
            if c == out_dim:
                ...
                proj = nn.Sequential(nn.Conv2d(c, out_dim, kernel_size=1, stride=1, bias=False), BatchNorm(out_dim),
                                     nn.ReLU(inplace=True))
            f = int(up_factors[i])
            if f == 1:
                up = Identity()
        ...
        for i in range(1, len(channels)):
            node = nn.Sequential(
                nn.Conv2d(out_dim * 2, out_dim, kernel_size=node_kernel, stride=1, padding=node_kernel // 2,
        ...
        for head in self.heads:
            ...
            if head_conv > 0:
                fc = nn.Sequential(
                    ...
                    nn.ReLU(inplace=True), nn.Conv2d(head_conv, classes, kernel_size=1, stride=1, padding=0, bias=True))
                if 'hm' in head:
